...
namespace EFSamurai.NUnitTest
{
    public class TddUnitTests
	{
		...
		{
			EfTddMethods.CreateSamurai(new Samurai() { Name = "Himura Kenshin", Quotes = new List<Quote>() { new() { Text = "You can always die. It's living that takes real courage.", QuoteStyle = QuoteStyle.Awesome } } });
			EfTddMethods.CreateSamurai(new Samurai() { Name = "Watsuki Nobuhiro", Quotes = new List<Quote>() { new() { Text = "New eras don't come about because of swords, they're created by the people who wield them.", QuoteStyle = QuoteStyle.Cheesy } } });
			EfTddMethods.CreateSamurai(new Samurai() { Name = "Laini Taylor", Quotes = new List<Quote>() { new() { Text = "Be a Samurai. Because you just never know what's behind the freaking sky.", QuoteStyle = QuoteStyle.Lame } } });
